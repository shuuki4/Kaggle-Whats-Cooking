...
class ClassifyNetwork(object) :
	def __init__(self, input, hidden1_size, hidden2_size, p, try_size) :
		## params
		...
		self.W2 = theano.shared(np.asarray(rng.uniform(low=-1.0/w2_bound, high=1.0/w2_bound, size=(hidden1_size, 20)), dtype=theano.config.floatX), name='W2', borrow=True)
		...
		self.b2 = theano.shared(np.asarray(np.zeros(20,), dtype=theano.config.floatX), name='b2', borrow=True)
		...
		select_array2 = T.cast(srng.binomial(n=1, p=1-p_val, size=(hidden1_size, 20)), theano.config.floatX)
		...
		select_vec2 = T.cast(srng.binomial(n=1, p=1-p_val, size=(20,)), theano.config.floatX)
		...
		out_temp2 = T.exp(hidden2_temp - hidden2_temp.max(axis=1, keepdims=True))
		self.output = out_temp2 / out_temp2.sum(axis=1, keepdims=True)

		# save parameters
		self.params = [self.W2, self.b2, self.W1, self.b1]
		self.paramins = [hidden1_size, hidden1_size, input_num, input_num]
